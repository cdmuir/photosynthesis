...
This vignette is designed to demonstrate how to use plantecophystools.
...
https://github.com/jstinzi/plantecophystools
...
#library(devtools)
#install_github("jstinzi/plantecophystools")

#Load package
library(photosynthesis)

#Load these packages as a fail-safe - they don't always load automatically
library(ggplot2)
library(minpack.lm)
library(nlme)
library(rlang)
...
#Print graphs to pdf. Note that this part is commented out
#to avoid issues with package compilation of the vignette.
#Remove "#"s to run.
...
#Fit regular Arrhenius curve
out <- fit_t_response_arrhenius(pars[pars$ID == "S1",],
                                varnames = list(Par = "J_max",
                                                Tleaf = "Curve_Tleaf"))

#Fit Heskel response removing the measurement at 40C
out <- fit_t_response_heskel(pars[pars$ID == "S1" & pars$Curve_Tleaf < 40,],
                                varnames = list(Par = "J_max",
                                                Tleaf = "Curve_Tleaf"))

#Print graph output of Heskel response
out[[3]]

#jpeg("tresponse.jpeg", width = 5, height = 5, res = 600, units = "in")
#out[[3]]
#dev.off()

#Fit many Medlyn et al. 2002 temperature response curves
fits <- fit_many(pars,
                 group = "ID",
                 funct = fit_t_response_medlyn,
                 varnames = list(Par = "V_cmax",
                                 Tleaf = "Curve_Tleaf"))

#Print graph of 1st replicate
fits[[1]][[3]]

#compile parameters
parameters <- compile_data(data = fits,
                           output_type = "dataframe",
                           list_element = 2)
```
...
#Fit the Medlyn model - select if fitting full model or not
...
#jpeg("g_mc.jpeg", height = 5, width = 5, res = 600, units = "in")
...
#Return graph
#jpeg("Hydra_vuln.jpeg", height = 5, width = 5, res = 600, units = "in")
#fit[[5]] 
#dev.off()
...
pars <- sensitivity_analysis(data = data[data$Q_2 == 1500, ],
                                  funct = fit_aci_response,
                                   ...
                                   values2 = seq(from = 0.2,
                                               ...
                                               by = 0.1))
#Graph Vcmax
a <- ggplot(pars, aes(x = gamma_star25, y = g_mc25, z = V_cmax))+
...
par2 <- compute_sensitivity(data = pars,
                            ...
                            test2_ref = 1)
#Plot sensitivity
jpeg("Figure 9.jpeg", height = 4, width = 4, res = 600,
     units = "in")
