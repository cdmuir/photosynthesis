...
fit_t_response <- function(data,
  ...
  if (TRUE %in% c(!model %in% c("Arrhenius",
                                "Kruse",
                                "Heskel",
                                "Medlyn",
                                "MMRT",
  ...
  if ("Arrhenius" %in% model) {
    ...
    try(models[["Arrhenius"]][[1]] <-  nlsLM(data = data,
                                         Par ~ Par25 * 
  ...
  if ("Heskel" %in% model) {
    ...
    try(models[["Heskel"]][[1]] <-  nlsLM(data = data,
                                          log(Par) ~ 
                                            t_response_heskel(a,
                                                              b,
                                                              c,
    ...
    models[["Heskel"]][[3]] <- ggplot(data, aes(x = T_leaf, y = Par)) +
      ...
      geom_smooth(
        method = "lm",
        formula = y ~ exp(t_response_heskel(T_leaf = x,
  ...
  if ("Kruse" %in% model) {
    ...
    data$T2 <- ((data$T_leaf) - 298.15) /
      ((data$T_leaf) * 298.15)
    try(models[["Kruse"]][[1]] <- nlsLM(data = data,
                                        log(Par) ~ 
    ...
    models[["Kruse"]][[3]] <- ggplot(data, aes(x = T_leaf, y = Par)) +
      ...
      geom_smooth(
        method = "lm",
        formula = y ~
          exp(t_response_arrhenius_kruse(T2 = ((x) - 298.15) /
                                           ((x) * 298.15),
        ...
        model_full[[i]] <- tryCatch(nlsLM(data = data,
                                          Par ~ Par25 *
                                            t_response_arrhenius_medlyn(Ea,
                                                                        ...
                                                                        T_leaf =
                                                                          T_leaf),
                                          start = list(Ea = coef(modeltest)[["Ea"]],
                                                       Hd = i * 1000,
                                                       dS = dSset,
                                                       Par25 = coef(modeltest)[["Par25"]]),
                                          lower = c(0, 0, 0, 0),
                                          upper = c(1000000, 2000000, 20000,
                                                    1.5 * max(data$Par)),
                                          control = nls.control(maxiter = 100)),
      ...
      models[["Medlyn"]][[3]] <- ggplot(data, aes(x = T_leaf, y = Par)) +
        ...
        geom_smooth(
          ...
          formula = y ~ I(models[["Medlyn"]][[2]]$Par25[1] *
                            t_response_arrhenius_medlyn(T_leaf = x,
  ...
  if ("MMRT" %in% model) {
    ...
    try(models[["MMRT"]][[1]] <- nlsLM(data = data,
  ...
  if ("Quadratic" %in% model) {
    ...
    try(models[["Quadratic"]][[1]] <-  nlsLM(data = data,
    ...
    models[["Quadratic"]][[3]] <- ggplot(data, aes(x = T_leaf, y = Par)) +
      ...
      geom_smooth(
        method = "lm",
        formula = y ~ (t_response_heskel(T_leaf = x,
    ...
    models[["Topt"]][[3]] <- ggplot(data, aes(x = T_leaf, y = Par)) +
      ...
      geom_smooth(
        ...
        formula = y ~ I(models[["Topt"]][[2]]$kopt[1] *
                          (t_response_arrhenius_topt(T_leaf = x,
