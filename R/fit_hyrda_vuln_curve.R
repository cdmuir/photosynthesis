...
#Extract model parameter values
  ...
  fit_out[[5]] <- ggplot(data, aes(x = psi, y = PLC)) +
    ggtitle(label = title) +
    geom_vline(xintercept = fit_out[[3]]$P50[1],
               size = 2,
               colour = "Blue") +
    geom_vline(xintercept = fit_out[[3]]$Pe[1],
               size = 2,
               colour = "Blue") +
    geom_vline(xintercept = fit_out[[3]]$Pmax[1],
               size = 2,
               colour = "Blue") +
    geom_vline(xintercept = fit_out[[3]]$P50[2],
               size = 2,
               colour = "Orange",
               linetype = "dashed") +
    geom_vline(xintercept = fit_out[[3]]$Pe[2],
               size = 2,
               colour = "Orange",
               linetype = "dashed") +
    geom_vline(xintercept = fit_out[[3]]$Pmax[2],
               size = 2,
               colour = "Orange",
               linetype = "dashed") +
    geom_smooth(method = "lm", aes(x = psi, y = PLC,
                                   colour = "Blue"), show.legend = TRUE, 
                formula = y ~ I(100 / (1 + exp(fit_out[[2]]$Value[2] * (x - fit_out[[2]]$Value[1])))),
                size = 2) + 
    geom_smooth(method = "lm", aes(x = psi, y = PLC,
                                   colour = "DarkOrange"), show.legend = TRUE,
                formula = y ~ I((1 - exp(- ((x / fit_out[[2]]$Value[4]) ^ fit_out[[2]]$Value[3]))) * 100),
                size = 2) +
  ...
  names(fit_out) <- c("Sig_Model", "Model_Parameters", "Model_Px Values",
                      "Wei_Model", "Graph")
