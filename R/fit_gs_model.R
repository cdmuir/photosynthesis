...
#' graphs[[3]]
#' }
fit_gs_model <- function(data, varnames = list(A_net = "A_net",
                                               C_air = "C_air",
                                               g_sw = "g_sw",
                                               RH = "RH",
                                               VPD = "VPD"),
  ...
  if (TRUE %in% c(!model %in% c("BallBerry",
                                "Leuning",
                                "Medlyn_partial",
                                "Medlyn_full"))) {
  ...
  if ("BallBerry" %in% model) {
    ...
    models[["BallBerry"]][[3]] <- ggplot(data, 
  ...
  if ("Medlyn_partial" %in% model) {
    ...
    try(models[["Medlyn_partial"]][[1]] <- nlsLM(data = data,
    ...
    models[["Medlyn_partial"]][[3]] <- ggplot(data, 
  ...
  if("Medlyn_full" %in% model) {
    ...
    #Fit model, assign to element 1
    try(models[["Medlyn_full"]][[1]] <- nlsLM(data = data,
    ...
    #Create graph, assign to element 3
    models[["Medlyn_full"]][[3]] <- ggplot(data, 
